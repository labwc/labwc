...
- [9. Use of AI-generated content](#use-of-ai-generated-content)
...
We do not accept contributions containing AI-generated content. Contributors may
be asked to explain the provenance of a contribution. This policy does not apply
to other uses of AI, such as researching APIs, performing static analysis, or
debugging, provided their output is not included in contributions.

This stance does not just apply to code, but also issues, discussions and PR
descriptions. If you are not a native English speaker and need assistance with
translations, please use a tool which doesn't use generative AI. Generative AI
often expands and twists the original message, altering its meaning.

The term AI is used here to include the field of Natural Language Processing
(NLP) which in turn includes Large Language Models (LLMs). Associated tools
include ChatGPT, Claude, Copilot, and so on.

The reason for this position relates to the following conerns:

1. **Copyright and provenance**: The legal status and provenance of AI-generated
   code is not sufficiently clear for us to be comfortable incorporating it into
   a GPL-2.0-only project.
2. **Quality**: Whilst some maintainers and contributors make significant use of
   AI tools in their professional lives, there is - with labwc - a sense that
   PRs generated by AI are annoying and that code created by AI is hard to
   follow. Furthermore, code review effort is usually the project's primary
   bottleneck, and the increase of AI-generated patches seems to reduce the
   number of people who can be trusted to reason about, review and uphold code
   quality. There also appears to be a temptation to lower the bar of code
   reviews for AI-generated stuff. We don't know why this is - maybe because the
   code appears clean and logical at a high level, or because the sense of
   ownership diminishes when one's fingerprints are not all over a patch.
3. **Community**: With the emergence of AI-assisted development we have observed
   that Pull Requests (PRs) have become more transactional with contributors
   asking an AI system to produce an implementation and submitting the result
   without the learning, discussion and personal investment that traditionally
   accompany participation in the project. This appears to breed a loss of
   purpose for those who are willing to mentor and help people grow.
4. **Absence of net-benefit**: Overall the maintainer feeling is that AI is not
   adding much value to labwc at this point, and that the disbenefits described
   in 1-3 far outweigh any gains.

To-date there are 7 commits with 'Assisted-by' tags covering very small
additions like libinput configuration options and documentation tidy-ups. These
are considered low risk against the points above and will therefore not be
reverted.

The points above are not derived from any political position relating to ethical
concerns like energy consumption and similar. No doubt individual maintainers
and contributors have thoughts on this, but it has not knowingly shaped the
policy.

This position can be revisited, should confidence grow that AI tools do not pose
copyright, quality and community concerns.

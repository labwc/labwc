		...
		int width, int height)
{
	 ...
	 */
	wlr_scene_buffer_set_dest_size(self->scene_buffer,
		width, height);
	self->width = width;
	self->height = height;

	if (self->scene_buffer->primary_output && self->active_scale > 0) {
		/* Immediately update the buffer if it is visible */
		_update_buffer(self, self->active_scale);
	} else {
		/* Force re-render on the next output_enter event */
		self->active_scale = 0;
	}
}

...
fill_mousebind(xmlNode *node, const char *context)
{
	...
	char mouse_event[256];
	char button[256];
	char direction[256];

	if (lab_xml_get_string(node, "button", button, sizeof(button))) {
		mousebind->button = mousebind_button_from_str(
			button, &mousebind->modifiers);
	}
	if (lab_xml_get_string(node, "direction", direction, sizeof(direction))) {
		mousebind->direction = mousebind_direction_from_str(
			direction, &mousebind->modifiers);
	}
	if (lab_xml_get_string(node, "action", mouse_event, sizeof(mouse_event))) {
		/* <mousebind button="" action="EVENT"> */
		mousebind->mouse_event = mousebind_event_from_str(mouse_event);
	}
...
/* Returns true if the parser should parse the node's children */
static bool

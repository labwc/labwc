...
fill_window_rule(char *nodename, char *content, struct parser_state *state)
{
	...
	/* Properties */
		...
		set_property(content,
			&state->current_window_rule->want_absorbed_modifier_release_events);
	...
	const char *branch_name)
{
	...
	if (!strcasecmp(nodename, "name.action")) {
		...
		if (state->current_inline_action) {
			wl_list_append(state->current_siblings,
...
fill_keybind(char *nodename, char *content, struct parser_state *state)
{
	...
	if (!strcmp(nodename, "key")) {
		...
		state->current_siblings = &state->current_keybind->actions;
...
fill_mousebind(char *nodename, char *content, struct parser_state *state)
{
	...
	if (!strcmp(nodename, "mousebind.context.mouse")) {
		...
		state->current_action = NULL;
		state->current_siblings = &state->current_mousebind->actions;
		return;
	} else if (!content) {
		return;
	}
...
entry(xmlNode *node, struct parser_state *state)
{
	...
	if ((state->in_keybind || state->in_mousebind) && state->in_action) {
	...
	if (state->in_libinput_category) {
		fill_libinput_category(name, content, state);
		return;
	}
	if (state->in_regions) {
		fill_region(name, content, state);
		return;
	}
...
rcxml_parse_xml(struct buf *b)
{
	...
	struct parser_state initial = {};
	xml_tree_walk(xmlDocGetRootElement(d), &initial);
	xmlFreeDoc(d);
	xmlCleanupParser();
}

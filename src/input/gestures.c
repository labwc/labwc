...
static void
gesture_tracker_begin(struct gesture_tracker *tracker, enum gesture_type type, uint8_t fingers)
{
...
static void
gesture_binding_match_and_run_action(struct seat *seat, struct gesture_tracker *tracker,
		struct lab_gesturebind *(*device_gbs)[MAX_FINGERS])
{
	...
	if (tracker->type == GESTURE_TYPE_SWIPE) {
		...
		if (fabs(tracker->dx) >= SWIPE_THRESHOLD || fabs(tracker->dy) >= SWIPE_THRESHOLD) {
			...
			struct lab_gesturebind *gesturebind =
				device_gbs[event][tracker->fingers];
			if (!gesturebind) {
				return;
			}
			// execute action
		...
		if (tracker->scale >= (1.0 + PINCH_THRESHOLD_FACTOR)
				|| tracker->scale <= (1.0 - PINCH_THRESHOLD_FACTOR)) {
 ...
 */
static void
gesture_tracker_end_and_match(struct seat *seat, struct gesture_tracker *tracker,
		struct lab_gesturebind *(*device_gbs)[MAX_FINGERS])
{
	gesture_binding_match_and_run_action(seat, tracker, device_gbs);
...
handle_pinch_update(struct wl_listener *listener, void *data)
{
	...
	if (gesture_tracker_check(&seat->gesture_state, GESTURE_TYPE_PINCH)) {
		gesture_tracker_update(&seat->gesture_state, event->dx, event->dy,
...
handle_swipe_update(struct wl_listener *listener, void *data)
{
	...
	if (gesture_tracker_check(&seat->gesture_state, GESTURE_TYPE_SWIPE)) {
		...
		if (fabs(tracker->dx) >= SWIPE_CONTINUROUS_THRESHOLD
				|| fabs(tracker->dy) >= SWIPE_CONTINUROUS_THRESHOLD) {
			struct wlr_input_device *dev = &event->pointer->base;
			struct input *input = dev->data;
			struct lab_gesturebind *(*device_gbs)[MAX_FINGERS] = input->gesture_binds;
			gesture_tracker_end_and_match(seat, &seat->gesture_state, device_gbs);

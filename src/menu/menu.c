...
static void
menu_configure(struct menu *menu, struct wlr_box anchor_rect, bool align_right)
{
	struct theme *theme = menu->server->theme;
	...
	struct wlr_xdg_positioner_rules rules = {
		 ...
		 * top edges aligned
		 */
		.anchor = align_right ? XDG_POSITIONER_ANCHOR_TOP_RIGHT
				: XDG_POSITIONER_ANCHOR_TOP_LEFT,
		.gravity = align_right ? XDG_POSITIONER_GRAVITY_BOTTOM_RIGHT
				: XDG_POSITIONER_GRAVITY_BOTTOM_LEFT,
		/* Slide or flip the menu when it overflows from the output */
	...
	/* Make submenus inherit their left/right alignment from parent */
	align_right = (box.x >= anchor_rect.x);
	...
	wl_list_for_each(item, &menu->menuitems, link) {
		...
		menu_configure(item->submenu, anchor_rect, align_right);
	}
}
...
create_pipe_menu(struct menu_pipe_context *ctx)
{
	...
	struct wlr_box anchor_rect = get_item_anchor_rect(
		ctx->server->theme, ctx->item);
	menu_configure(pipe_menu, anchor_rect, /* align_right */ true);

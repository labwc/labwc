...
/* Group */
static void
ws_create_workspace_handle_transaction_destroy(struct wl_listener *listener, void *data)
{
	struct ws_create_workspace_event *ev =
		wl_container_of(listener, ev, on.transaction_destroy);
	wl_list_remove(&ev->on.transaction_destroy.link);
	free(ev->name);
	free(ev);
}
		...
		struct wl_resource *resource, const char *name)
{
	...
	wl_signal_add(&transaction->events.destroy, &ev->on.transaction_destroy);
}
...
lab_cosmic_workspace_group_destroy(struct lab_cosmic_workspace_group *group)
{
	...
	/* Cancel pending transactions involving this group */
	struct lab_transaction *trans, *trans_tmp;

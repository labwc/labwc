...
theme_builtin(struct theme *theme, struct server *server)
{
	...
	theme->menu_label_text_justify = parse_justification("Left");
...
entry(struct theme *theme, const char *key, const char *value)
{
	...
	if (match_glob(key, "menu.label.text.justify")) {
		theme->menu_label_text_justify = parse_justification(value);
	}

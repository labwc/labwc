...
view_cascade(struct view *view)
{
	...
	while (candidate_updated) {
		...
		for_each_view(other_view, &view->server->views,
			...
			/*
			 * If the candidate covers an existing view which is
			 * not already covered, shift the candidate to bottom-right.
			 */
			if (box_contains(&candidate, &other)
					&& !box_contains(&covered, &other)) {
